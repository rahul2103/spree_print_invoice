  ...
  if Spree::Core::Engine.backend_available?
    ...
    Rails.application.config.spree_backend.tabs[:order].add(
      ::Spree::Admin::Tabs::TabBuilder.new(::Spree.t(:documents, scope: [:print_invoice]), lambda { |resource|
        ::Spree::Core::Engine.routes.url_helpers.admin_order_bookkeeping_documents_path(resource)
      })
    ...
    Rails.application.config.spree_backend.main_menu.add_to_section('settings',
                                                                    ::Spree::Admin::MainMenu::ItemBuilder.new('print_invoice.settings',
                                                                                                              ::Spree::Core::Engine.routes.url_helpers.edit_admin_print_invoice_settings_path)
