module Spree
  module Admin
    class PrintInvoiceSettingsController < ResourceController
      before_action :find_or_create_setting, only: %i[index update]
      ...
      def print_invoice_setting_params
        params.require(:print_invoice_setting).permit(:next_number, :logo_path, :page_size, :page_layout, :footer_left, :footer_right,
                                                      :return_message, :anomaly_message, :use_footer, :use_page_numbers, :logo_scale, :font_face, :font_size, :store_pdf, :storage_path, :store_id, :logo)
      end
    end
  end
end
